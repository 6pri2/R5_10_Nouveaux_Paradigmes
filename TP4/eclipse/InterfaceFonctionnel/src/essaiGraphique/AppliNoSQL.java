...
public class AppliNoSQL {
    ...
    public ArrayList<Coureur> lireDonnees(String nom, String prenom, String anneNaiss, String annePrem ) {
    	...
    	Bson filtre = Filters.and(
    	        Filters.regex("nom", ".*" + nom + ".*"), 
    	        Filters.regex("prenom", ".*" + prenom + ".*"),
    	        Filters.regex("annee_naissance", ".*" + anneNaiss + ".*"),
    	        Filters.regex("annee_prem", ".*" + annePrem + ".*"));
        FindIterable<Document> docs = collection.find().projection(new Document("_id", 0));
        for (Document d : docs) {
            liste.add(new Coureur(d.get("n_coureur").toString(),d.getString("nom"),d.getString("prenom"),d.get("annee_naissance").toString(),d.get("annee_prem").toString()));
        }
        return liste;
    }
